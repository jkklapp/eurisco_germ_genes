...
hadoop jar $mrstream -D mapreduce.job.maps=40 -input eurisco_germ_$date/species -mapper getNucIdsMap_$date.py  -file getNucIdsMap_$date.py -reducer justPrint_$date.py -file justPrint_$date.py -output eurisco_germ_$date/ids
if [ $? != 0 ]; then
        echo "MR2 failed!"
...
hadoop jar $mrstream -D mapreduce.job.maps=40 -D mapreduce.map.memory.mb="8192" -D mapred.child.java.opts="-Xmx512M" -input eurisco_germ_$date/ids -mapper getFullRecFromId_$date.py -file getFullRecFromId_$date.py -reducer storeInMongo_$date.py -file storeInMongo_$date.py -output eurisco_germ/final_$date
if [ $? != 0 ]; then
        echo "MR3 failed!"
